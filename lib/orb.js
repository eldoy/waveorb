...
module.exports = function(app, client = {}, params = {}) {
  // Set up language
  const method = client.req?.method?.toUpperCase()
  const cookieLang = client.req?.cookie('lang')
  const defaultLang = process.env.WAVEORB_LANG
    || _.get(app, 'config.env.lang')
    || _.get(app, 'config.routes.lang')
    || 'en'
  const lang = params.lang
    || i18n.getLang(client.req?.pathname)
    || method === 'POST' && cookieLang
    || defaultLang
