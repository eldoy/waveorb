...
module.exports = async function(options = {}) {
  ...
  if (typeof hooks.init == 'function') await hooks.init(app)
  ...
  const assetRoot = fpath.join(APP_ROOT, path, 'assets')
  const assetList = tree(`${path}/assets`).map(x => x.replace(assetRoot, ''))
  const assets = new Set(assetList)
  ...
  for (const model in actions) {
    ...
    for (const name in items) {
      if (_.isPlainObject(items[name]) && _.isEmpty(items[name])) {
        const fn = DEFAULT_FUNCTIONS[name]
        if (fn) {
          items[name] = fn(model)
        } else {
          delete items[name]
  ...
  // Run load hook before returning
  if (typeof hooks.load == 'function') await hooks.load(app)
