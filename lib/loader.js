...
module.exports = async function (options = {}) {
  ...
  if (mode == 'development' && !process.env.WAVEORB_CMD) {
    console.time('Waveorb :: app loaded')
  }
  ...
  function onload(props) {
    ...
    if (ext == 'md') {
      return titled(base, tools.markdown(content))
    }
    if (ext == 'html') {
  ...
  if (mode == 'development' && !process.env.WAVEORB_CMD) {
    console.timeEnd('Waveorb :: app loaded')
  }
