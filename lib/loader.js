...
module.exports = async function (options = {}) {
  ...
  const validations = load(`${path}/validations`)
  const collections = load(`${path}/collections`)
  const apis = load(`${path}/apis`)
  const models = load(`${path}/models`)
  const transformers = load(`${path}/transformers`)
  const converters = load(`${path}/converters`)
  ...
  const components = load(`${path}/components`, htmlLoadOptions)
  const elements = load(`${path}/elements`, htmlLoadOptions)
  ...
  const layouts = load(`${path}/layouts`, htmlLoadOptions)
  const pages = load(`${path}/pages`, htmlLoadOptions)
