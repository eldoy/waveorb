...
function getRouteName(name, $) {
  ...
  if (!_.isEmpty(routemap)) {
    ...
    // Direct mapping
    let mapping = routemap[routeName]

    // Dynamic mapping
    if (!mapping) {
      const route = Object.entries(routemap).find(([key, val]) => {
        key = key.slice(1).replace(/\.html$/, '')
        const pattern = key.split('/').map(x => x.startsWith('_') ? '(.+)' : x).join('/')
        const match = new RegExp(pattern, 'g').test(name)
        ...
        if (match) {
          ...
          key.split('/').forEach((val, i) => {
            if (val.startsWith('_')) {
              $.req.query[val.slice(1)] = names[i]
            }
          })
        }
        return match
      })
      if (route) mapping = route[1]
    }
