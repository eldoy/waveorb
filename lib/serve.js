...
module.exports = async function(options = {}, app) {
  ...
  for (const m in app.middleware) {
    ...
    typeof fn == 'function' && server.use(function(req, res) {
      const client = { req, res, server }
      req.store = orb(app, client)
      return fn(req.store)
    })
  }
  ...
  async function dispatch(fn, client, params) {
    ...
    // Store may have been set in middleware
    const store = client.req?.store
    const $ = { ...store, ...orb(app, client, params) }
