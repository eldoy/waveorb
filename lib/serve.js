...
module.exports = async function(options = {}, app) {
  ...
  async function dispatch(fn, client, params) {
    ...
    try {
      let result = typeof (hook = app.hooks.dispatch) == 'function' && await hook($)
      if (typeof result != 'undefined') return result

      const current = await fn($)

      result = typeof (hook = app.hooks.result) == 'function' && await hook(current, $)
      if (typeof result != 'undefined') return result
      ...
      result = typeof (hook = app.hooks.error) == 'function' && await hook(error, $)
      if (typeof result != 'undefined') return result
  ...
  server.get('*', async function(req, res) {
    const result = typeof (hook = app.hooks.asset) == 'function' && await hook(req, res)
    if (typeof result != 'undefined') return result
